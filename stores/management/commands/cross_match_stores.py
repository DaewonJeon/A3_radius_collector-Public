"""
세 가지 편의점 데이터 교차 매칭 스크립트 V3
1. public_data.csv (소상공인상권 CSV) - 504개
2. SeoulRestaurantLicense (영등포구 휴게음식점 인허가 OpenAPI) - 310개  
3. YeongdeungpoConvenience (다이소 기반 추출) - 462개
...
class Command(BaseCommand):
    help = '세 가지 편의점 데이터 교차 매칭 V3 (주소 일치 시 2차 검증)'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("🔍 세 가지 편의점 데이터 교차 매칭 V3"))
        self.stdout.write(self.style.SUCCESS("=" * 70))
        ...
        # 5. 결과 출력
        self.stdout.write("\n" + "=" * 70)
        self.stdout.write(self.style.SUCCESS(f"🎯 최종 매칭: {len(matched_stores)}개 고유 편의점"))
        ...
        if matched_stores:
            ...
            # 상세 출력
            self.stdout.write("\n" + "-" * 70)
            self.stdout.write("📌 매칭된 편의점:")
            self.stdout.write("-" * 70)
            
            for i, store in enumerate(matched_stores[:30], 1):
                self.stdout.write(f"\n[{i}] {store['이름']}")
                self.stdout.write(f"    주소: {store['주소']}")
                self.stdout.write(f"    좌표: ({store['위도']}, {store['경도']})")
                self.stdout.write(f"    매칭: {store['매칭이유']}")
            
            if len(matched_stores) > 30:
                self.stdout.write(f"\n... 외 {len(matched_stores) - 30}개")
            ...
            name_match = sum(1 for s in matched_stores if '이름매칭' in s['매칭이유'])
            addr_match = sum(1 for s in matched_stores if '주소매칭' in s['매칭이유'])
            coord_match = sum(1 for s in matched_stores if '좌표매칭' in s['매칭이유'])
            secondary = sum(1 for s in matched_stores if '2차검증' in s['매칭이유'])
